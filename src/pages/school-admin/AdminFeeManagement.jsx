...
const EMPTY_TYPE = { name: '', description: '', colorCode: '#6366f1', isMandatory: true, defaultAmount: 0, applicableClassIds: [], applicableClassNames: [] };
const EMPTY_STRUCTURE = { name: '', academicYear: '', term: '', applyToAllClasses: true, applicableClasses: [], applicableClassNames: [], feeItems: [], status: 'draft', totalAmount: 0 };
...
export default function AdminFeeManagement() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      setFeeTypes(ft);
      setStructures(fs);
      setClasses(cl.filter(c => !c.isArchived));
    } catch (e) {}
    setLoading(false);
  }
  ...
  function openEditType(t) {
    setEditingType(t);
    setTypeForm({
      name: t.name, description: t.description || '', colorCode: t.colorCode || '#6366f1',
      isMandatory: t.isMandatory !== false, defaultAmount: t.defaultAmount || 0,
      applicableClassIds: t.applicableClassIds || [], applicableClassNames: t.applicableClassNames || [],
    });
    setShowTypeDialog(true);
  }

  function toggleClassForType(cls) {
    setTypeForm(f => {
      const ids = f.applicableClassIds || [];
      const names = f.applicableClassNames || [];
      if (ids.includes(cls.id)) {
        return { ...f, applicableClassIds: ids.filter(id => id !== cls.id), applicableClassNames: names.filter(n => n !== cls.className) };
      }
      return { ...f, applicableClassIds: [...ids, cls.id], applicableClassNames: [...names, cls.className] };
    });
  }
  ...
  function openEditStructure(s) {
    ...
    setStructureForm({
      name: s.name, academicYear: s.academicYear || '', term: s.term || '',
      applyToAllClasses: s.applyToAllClasses !== false,
      applicableClasses: s.applicableClasses || [], applicableClassNames: s.applicableClassNames || [],
      feeItems: s.feeItems || [], status: s.status || 'draft', totalAmount: s.totalAmount || 0,
    });
    setShowStructureDialog(true);
  }

  function toggleClassForStructure(cls) {
    setStructureForm(f => {
      const ids = f.applicableClasses || [];
      const names = f.applicableClassNames || [];
      if (ids.includes(cls.id)) {
        return { ...f, applicableClasses: ids.filter(id => id !== cls.id), applicableClassNames: names.filter(n => n !== cls.className) };
      }
      return { ...f, applicableClasses: [...ids, cls.id], applicableClassNames: [...names, cls.className] };
    });
  }
                ...
                <CardContent className="pt-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2">
                      <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: ft.colorCode || '#6366f1' }} />
                  ...
                  {(ft.applicableClassIds || []).length > 0 && (
                    <p className="text-xs text-muted-foreground mt-1">{(ft.applicableClassNames || []).join(', ')}</p>
                  )}
                </CardContent>
              </Card>
            ))}
              ...
              <Card key={s.id}>
                <CardContent className="pt-4">
                  ...
                  {s.applyToAllClasses
                    ...
                    : <p className="text-xs text-muted-foreground">{(s.applicableClassNames || []).join(', ')}</p>
                  }
                </CardContent>
              </Card>
            ))}
      ...
      <Dialog open={showTypeDialog} onOpenChange={setShowTypeDialog}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            {classes.length > 0 && (
              <div>
                <Label className="text-sm mb-2 block">Applicable Classes <span className="text-muted-foreground font-normal">(leave blank for all)</span></Label>
                <div className="grid grid-cols-2 gap-1 max-h-40 overflow-y-auto border rounded-md p-2">
                  {classes.map(cls => {
                    const selected = (typeForm.applicableClassIds || []).includes(cls.id);
                    return (
                      <button key={cls.id} type="button"
                        onClick={() => toggleClassForType(cls)}
                        className={`flex items-center gap-2 px-2 py-1 rounded text-sm text-left transition-colors ${selected ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`}>
                        {selected && <Check className="w-3 h-3 flex-shrink-0" />}
                        <span className={selected ? '' : 'ml-5'}>{cls.className}</span>
                      </button>
                    );
                  })}
                </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                <input type="checkbox" id="allClasses" checked={structureForm.applyToAllClasses} onChange={e => setStructureForm(f => ({ ...f, applyToAllClasses: e.target.checked, applicableClasses: [], applicableClassNames: [] }))} />
                <Label htmlFor="allClasses">Apply to all classes</Label>
              </div>
              {!structureForm.applyToAllClasses && classes.length > 0 && (
                <div>
                  <Label className="text-sm mb-2 block">Select Classes *</Label>
                  <div className="grid grid-cols-2 gap-1 max-h-40 overflow-y-auto border rounded-md p-2">
                    {classes.map(cls => {
                      const selected = (structureForm.applicableClasses || []).includes(cls.id);
                      return (
                        <button key={cls.id} type="button"
                          onClick={() => toggleClassForStructure(cls)}
                          className={`flex items-center gap-2 px-2 py-1 rounded text-sm text-left transition-colors ${selected ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`}>
                          {selected && <Check className="w-3 h-3 flex-shrink-0" />}
                          <span className={selected ? '' : 'ml-5'}>{cls.className}</span>
                        </button>
